...
- AWS에 대해 경험이 부족하여 이번 과제를 통해 공부하며 부하와 관해 ELB, ECS 등을 공부해 보았습니다.
 
- DOCKER를 사용하여 데이터베이스도 관리하고 테이블도 구현하면서 동시에 JAVA로 구성된 파일도 같이 실행시키는 과제를 진행하면서 ECS라는 서비스에 대해 알게 되었습니다. 

- ELB(Elastic Load Balancing)은 하나의 가용영역, EC2에 수신되는 트래픽을 분산하는 서비스며 ELB에 대해 알아보며 ELB는 EC2의 부하 분산을 하며 Auto Scaling과 연동하여 추가된 EC2를 부하분산에 포함시키거나 제외한다는 것을 알 수 있었습니다.
- 부하를 분산하기 위해서 작업을 처리하는 순서도 같이 관리하는지 궁금하여 공부하다 알고리즘 수업시간에 배운 그리디 알고리즘과 같은 작업스케쥴링에 대해 떠올리게 되었습니다.
