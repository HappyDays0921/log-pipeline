...
- docker compose up -d 이후에 untitled/output에 chart 이미지가 생성되지않는것을 해결중에 있습니다.

- 처음 에러 발생시 php연결상태에서 local이 문제가 될 줄 몰랐습니다. 또한 다른 프로젝트를 진행함에 있어서 컨테이너 이름의 중복으로 인해 compose파일을 작성하는데 애를 먹었습니다. 다만 ai도구를 활용하여 local이 의미하는점이 다르다는것을 알게 되었으며 docker-compose에 정의된 서비스 이름으로 통신해야 한다는 것을 확인하였습니다.
...
- AWS에 대해 경험이 부족하여 이번 과제를 통해 공부하며 부하와 관해 ELB에 대해 공부해 보았습니다.

- AWS ELB서비스가 하나의 가용영역, EC2에 수신되는 트래픽을 자동으로 분산하며 시간에 따라 들어오는 트래픽 변화에 맞추어 자동으로 확장하는하는 것을 알게 되었습니다.
- 
- 부하를 어떻게 분산시키는지 궁금하여 AWS ELB 서비스 공식문서에서 찾아보니, 실제로는 라운드로빈이라는 알고리즘 방식으로 요청을 분산하거나 Shortest Job First 스케줄링 기법을 실제로 사용한다는 점을 알게 되었습니다. 이 과정에서 알고리즘 수업시간에 배웠던 작업스케줄링에 대해 다시 떠오르게 되었습니다.

- EBL에 대해 공부하며 VPC라는 개념도 공부하게 되었습니다. 사설네트워크를 활용하여 네트워크망을 구성하고 내부에 리소스를 탑재할수 있는 서비스가 VPC이고, 대표적으로 EC2, ELB가 있다는 것을 알게 되었습니다.
- VPC를 쪼갠것을 Subnet이라 하며 이 서브넷은 같은 네트워크 테이블에서 통신할 경우 라우팅 테이블이 필요하지 않다는 점을 알게 되었습니다.
